...
export function DemoControls({ cierreId, fecha, tieneMovimientos, esDemo }: { cierreId: number; fecha: string; tieneMovimientos: boolean; esDemo: boolean }) {
  ...
  return <>
    <section className={`mb-3 rounded-xl border p-4 sm:flex sm:flex-wrap sm:items-center sm:justify-between sm:gap-4 ${esDemo ? "border-amber-300 bg-amber-50" : "border-dashed border-slate-300 bg-white/70"}`}>
      <div>{esDemo ? <><p className="text-xs font-bold uppercase tracking-wider text-amber-700">Escenario demo</p><h2 className="mt-1 text-sm font-bold text-amber-950">Estás usando datos de ejemplo</h2><p className="mt-1 text-xs leading-5 text-amber-800">Estos datos sirven para probar CierreAI y no son tus datos reales.</p></> : <><h2 className="text-sm font-bold text-slate-900">Modo demo</h2><p className="mt-1 text-xs leading-5 text-slate-600">Carga operaciones de ejemplo en este cierre. El análisis se ejecuta por separado.</p></>}</div>
      <div className="mt-3 flex flex-col gap-2 sm:mt-0 sm:flex-row">
        {esDemo ? <><button type="button" disabled={pending} onClick={() => ejecutar(() => restablecerEscenarioDemo(cierreId))} className="rounded-lg border border-amber-400 bg-white px-3 py-2.5 text-sm font-bold text-amber-800 disabled:opacity-60">{pending ? "Procesando…" : "Restablecer escenario demo"}</button><button type="button" disabled={pending} onClick={() => setConfirmacion("empezar")} className="rounded-lg bg-amber-900 px-3 py-2.5 text-sm font-bold text-white disabled:opacity-60">Empezar con mis datos</button></> : <button type="button" disabled={pending} onClick={cargar} className="rounded-lg border border-blue-300 bg-white px-3 py-2.5 text-sm font-bold text-blue-700 disabled:opacity-60">{pending ? "Cargando escenario…" : "Cargar escenario demo"}</button>}
      </div>
    ...
    {confirmacion && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/45 p-4" role="dialog" aria-modal="true" aria-labelledby="vaciar-title" aria-busy={operacionPendiente}><div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl"><h2 id="vaciar-title" className="text-xl font-bold text-slate-950">{confirmacion === "empezar" ? "Empezar con mis datos" : "Vaciar cierre"}</h2><p className="mt-3 text-sm leading-6 text-slate-700">{confirmacion === "empezar" ? "Se eliminarán los movimientos y análisis de ejemplo de este cierre para que puedas empezar desde cero. Esta acción no afecta otros cierres." : "Se eliminarán todas las ventas, gastos, pagos, análisis y el efectivo contado de este cierre. Esta acción no se puede deshacer y no afectará otros días."}</p>{operacionPendiente && <p role="status" className="mt-4 flex items-center gap-2 text-sm font-bold text-blue-700"><span className="size-4 animate-spin rounded-full border-2 border-blue-200 border-t-blue-700" />Vaciando el cierre…</p>}<div className="mt-6 flex justify-end gap-2"><button type="button" disabled={operacionPendiente} onClick={() => setConfirmacion(null)} className="rounded-lg border border-slate-300 px-4 py-2.5 text-sm font-bold text-slate-700 disabled:cursor-not-allowed disabled:opacity-60">Cancelar</button><button type="button" disabled={operacionPendiente} onClick={confirmar} className="rounded-lg bg-red-700 px-4 py-2.5 text-sm font-bold text-white disabled:cursor-wait disabled:opacity-60">{operacionPendiente ? "Vaciando…" : confirmacion === "empezar" ? "Vaciar y empezar" : "Vaciar cierre"}</button></div></div></div>}
  </>;
}
