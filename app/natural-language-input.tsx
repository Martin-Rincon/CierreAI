  ...
  return <section className="mb-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" aria-labelledby="carga-ia-title">
    <div className="grid gap-5 lg:grid-cols-[1fr_0.9fr]">
      ...
      {movimiento ? <article className="rounded-xl border border-blue-200 bg-blue-50/60 p-4">
        ...
        <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:flex-wrap"><form action={confirmarMovimientoIa}><input type="hidden" name="cierre_id" value={cierreId} /><input type="hidden" name="submission_id" value={crypto.randomUUID()} /><input type="hidden" name="movimiento" value={JSON.stringify(movimiento)} /><SubmitButton idle="Confirmar y guardar" pending="Guardando…" className="w-full rounded-lg bg-emerald-600 px-3 py-2.5 text-sm font-bold text-white sm:w-auto" /></form><button type="button" onClick={() => setMovimiento(null)} className="rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm font-bold text-slate-700">Editar o cancelar</button></div>
